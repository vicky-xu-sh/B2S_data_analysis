...
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16              # passed to AMICA as max_threads
#SBATCH --mem=64G
#SBATCH --output=/scratch/st-ssfels-1/vickywx/logs_amica/%x_%j.out
#SBATCH --error=/scratch/st-ssfels-1/vickywx/logs_amica/%x_%j.err
...
export AMICA_DIR="${BASE}/amica_dir"
...
# ---------------------------------------------------------------------------
mkdir -p /scratch/st-ssfels-1/vickywx/logs_amica
mkdir -p "${OUTPUT_DIR}"
mkdir -p "${AMICA_DIR}"
...
matlab -nodisplay -nosplash -nodesktop -r \
    "try; run('/scratch/st-ssfels-1/vickywx/B2S_data_analysis/src/matlab/cluster/amica_hpc.m'); catch e; fprintf('ERROR: %s\n', e.message); exit(1); end; exit(0);"
